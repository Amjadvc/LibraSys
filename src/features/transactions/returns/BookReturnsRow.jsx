...
import ConfirmReturnTransaction from './ConfirmReturnTransaction';

function BookReturnsRow({ transaction }) {
  const {
    id,
    book,
    customer,
  ...
  return (
    <Table.Row className="hover:bg-background-50">
      ...
      {/* Actions */}
      <Modal>
        <Modal.Open opens="return">
          <Button variant="return">Return</Button>
        </Modal.Open>

        <Modal.Window name="return" type="select">
          <ConfirmReturnTransaction transaction={transaction} />
        </Modal.Window>
      </Modal>
    </Table.Row>
  );
